...
interface EditFieldModalProps {
  ...
  fieldName: string;
  handleClose: () => void;
  handleOk: (newInput: number, rowId: string, fieldName: string) => void;
}
...
const EditFieldNumberModal: React.FC<EditFieldModalProps> = ({
  open,
  data,
  rowId,
  fieldName,
  ...
  const [newInput, setNewInput] = useState<number>(data);
      ...
      <Box sx={style}>
        <Box sx={{ mt: 4, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
            <TextField
              ...
              value={newInput}
              onChange={(newVal) => {
                setNewInput(Number(newVal.target.value));
              }}
            />
